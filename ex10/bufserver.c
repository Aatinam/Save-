  ...
  while (1) {
    ...
    if ((fd = accept(listenfd, (struct sockaddr *)&peer, &socklen)) < 0) {
      ...
      while ((nbytes = read(fd, after, room)) > 0) {
        ...
        where = find_network_newline(buf, inbuf); //called find_network on
                                                      //the start of the array
                                                      //to check first inbuf bytes
